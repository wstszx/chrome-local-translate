...
/**
 * 读一遍「Gemini Nano 硬件门槛」相关的可探测信号，给 UI 一个具体结论，
 * 而不是让用户自己去对照「>4GB 显存，或 16GB 内存 + 4 核」这行字。
 *
 * 注意：浏览器 JS 拿不到显存大小，只能拿到：
 *   - navigator.hardwareConcurrency：CPU 逻辑核心数
 *   - navigator.deviceMemory       ：近似内存（部分 Chrome 版本裁到 8 就封顶，仅供参考）
 *   - navigator.storage.estimate() ：当前 origin 可用的存储配额（不是磁盘总剩余空间，
 *                                    但配额不足时基本可以确定「肯定装不下」）
 * 所以这里只做「明显不满足」的排除法判断，给不出「一定满足」的结论——
 * 满不满足显存要求，最终还是要看 chrome://components 里模型是不是真的下载成功。
 */
export async function checkNanoHardware() {
  const cores = typeof navigator !== 'undefined' ? navigator.hardwareConcurrency : undefined;
  const memoryGB = typeof navigator !== 'undefined' ? navigator.deviceMemory : undefined;
  let freeGB = null;
  let quotaGB = null;
  try {
    if (typeof navigator !== 'undefined' && navigator.storage && typeof navigator.storage.estimate === 'function') {
      const est = await navigator.storage.estimate();
      if (typeof est.quota === 'number') quotaGB = est.quota / 1024 ** 3;
      if (typeof est.quota === 'number' && typeof est.usage === 'number') {
        freeGB = (est.quota - est.usage) / 1024 ** 3;
      }
    }
  } catch (err) {
    /* 部分环境（尤其是隐身模式）会拒绝 estimate()，忽略即可 */
  }

  const reasons = [];
  // CPU + 内存这一条是「或」的关系里比较容易在 JS 里验证的一半（另一半是显存，测不出来）
  const cpuMemOk = typeof cores !== 'number' || typeof memoryGB !== 'number' ? null : cores >= 4 && memoryGB >= 16;
  if (cpuMemOk === false) reasons.push(`CPU/内存偏低（${cores || '?'} 核 · ${memoryGB || '?'}GB），如果这台机器也没有 >4GB 独立显存，Nano 大概率装不上`);
  // 配额是「这个 origin 能用多少」，不是「磁盘还剩多少」，但配额都不够 22GB 时基本能排除
  const quotaLikelyOk = quotaGB == null ? null : quotaGB >= 22;
  if (quotaLikelyOk === false) reasons.push(`浏览器给这个来源的存储配额只有约 ${quotaGB.toFixed(1)}GB，装不下约 2~4GB 的模型 + 缓存所需空间`);

  return { cores, memoryGB, freeGB, quotaGB, cpuMemOk, quotaLikelyOk, reasons };
}
...
async function nanoAvailability() {
  ...
  // 不代表硬件不支持。Chrome 启用 MTP 推测解码时可能要求 samplingMode: 'most-predictable'。
  // 所以按顺序多问几次：
  //   ① 带 samplingMode: 'most-predictable' 查询
  //   ② 无参 availability()
  //   ③ 与 create() 一致的模态声明再问（带/不带 samplingMode）
  //   ④ 老版 capabilities() 兜底。
  ...
  try {
    if (typeof LM.availability === 'function') {
      let first = null;
      try {
        first = await LM.availability({ samplingMode: 'most-predictable' });
      } catch (err) {
        first = null;
      }
      ...
      if (first && first !== 'unavailable') return first;
      try {
        const second = await LM.availability({ samplingMode: 'most-predictable', ...expected });
        if (second && second !== 'unavailable') return second;
      } catch (err) {
        /* 继续往下问 */
      }
      try {
        const third = await LM.availability(expected);
        if (third && third !== 'unavailable') return third;
      } catch (err) {
        /* 落到 capabilities 兜底 */
      }
...
export async function selftest({ source = 'en', target = 'zh', destroy = true } = {}) {
  ...
  if (support.nano) {
    ...
    try {
      ...
      let session = null;
      let lastErr = null;
      for (const opts of testAttempts) {
        try {
...
export async function ensureNanoModel({ onDownload, signal } = {}) {
  ...
  const attempts = [
    { samplingMode: 'most-predictable', monitor, signal },
    { samplingMode: 'most-predictable', ...expected, monitor, signal },
    { temperature: 0, topK: 1, monitor, signal },
    { topK: 1, monitor, signal },
...
export async function getNanoSession(cfg, { onDownload, signal } = {}) {
  ...
  const promise = (async () => {
    ...
    //
    // 采样参数兼容性（MTP 推测解码 & Prompt API 规范）：
    // 1) Chrome 开启 MTP 推测解码（Speculative Decoding）时，强制要求指定兼容采样选项：
    //    i.e. samplingMode: 'most-predictable'，或 topK: 1，或 temperature: 0；
    //    若缺省或配置了非贪婪采样参数，Chrome 会抛出 NotSupportedError
    //    "The sampling options are incompatible with speculative decoding (MTP)..."；
    // 2) 扩展环境中 temperature 与 topK 必须成对出现且不超过 params() 上下限；
    // 3) 按照分层回退策略逐级尝试：优先尝试采样参数或 most-predictable，遭遇版本不兼容或 MTP 限制时
    //    自动降级重试，确保任何 Chrome 都可以顺利创建会话。
    ...
    const attempts = [
      sampling ? { ...base, ...sampling } : { ...base, samplingMode: 'most-predictable' },
      { ...base, samplingMode: 'most-predictable' },
      { ...base, samplingMode: 'most-predictable', temperature: 0, topK: 1 },
      { ...base, temperature: 0, topK: 1 },
      { ...base, topK: 1 },
      { ...base, temperature: 0 },
      { initialPrompts: [{ role: 'system', content: systemPrompt }], monitor },
      { samplingMode: 'most-predictable', monitor },
      { temperature: 0, topK: 1, monitor },
      { monitor },
    ];
    let lastErr;
    for (const raw of attempts) {
